...
class _SettingsPageState extends State<SettingsPage> {
          ...
          return ListView(
            children: [
              ...
              _buildSettingItem(
                ...
                title: 'Theme',
                subtitle: _getThemeDisplayName(effectivePreferences.themeMode),
                onTap: () => _showThemeDialog(context, effectivePreferences),
              ),
              _buildSettingItem(
                context,
                icon: Icons.language,
                title: 'Language',
                subtitle: _getLanguageDisplayName(effectivePreferences.language),
                onTap: () => _showLanguageDialog(context, effectivePreferences),
              ),
              ...
              _buildSettingItem(
                ...
                title: 'Snooze Duration',
                subtitle: '${effectivePreferences.reminderSnoozeDuration} minutes',
                onTap: () => _showSnoozeDialog(context, effectivePreferences),
              ),
  ...
  // FIXED: Pass the entire preferences object instead of just themeMode
  void _showThemeDialog(BuildContext context, UserPreferencesEntity currentPreferences) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Select Theme'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildThemeOption(context, ThemeMode.system, 'System Default', currentPreferences),
            _buildThemeOption(context, ThemeMode.light, 'Light', currentPreferences),
            _buildThemeOption(context, ThemeMode.dark, 'Dark', currentPreferences),
          ],
        ),
      ),
    );
  ...
  // FIXED: Update using current preferences instead of creating new ones
  Widget _buildThemeOption(BuildContext context, ThemeMode themeMode, String label, UserPreferencesEntity currentPreferences) {
    return ListTile(
      title: Text(label),
      leading: Radio<ThemeMode>(
        value: themeMode,
        groupValue: currentPreferences.themeMode,
        onChanged: (value) {
          Navigator.pop(context);
          if (value != null) {
            // Update theme using the dedicated event
            context.read<ProfileBloc>().add(UpdateThemeModeEvent(themeMode: value));
          }
        },
      ),
  ...
  // FIXED: Pass the entire preferences object instead of just language
  void _showLanguageDialog(BuildContext context, UserPreferencesEntity currentPreferences) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Select Language'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildLanguageOption(context, 'english', 'English', currentPreferences),
            _buildLanguageOption(context, 'spanish', 'Español', currentPreferences),
            _buildLanguageOption(context, 'french', 'Français', currentPreferences),
          ],
        ),
      ),
    );
  ...
  // FIXED: Update using current preferences instead of creating new default ones
  Widget _buildLanguageOption(BuildContext context, String language, String label, UserPreferencesEntity currentPreferences) {
    return ListTile(
      title: Text(label),
      leading: Radio<String>(
        value: language,
        groupValue: currentPreferences.language,
        onChanged: (value) {
          Navigator.pop(context);
          if (value != null) {
            // Update the current preferences with the new language
            _updatePreference(
              context,
              currentPreferences.copyWith(language: value), // FIXED: Use current preferences
            );
          }
        },
      ),
  ...
  // FIXED: Pass the entire preferences object instead of just duration
  void _showSnoozeDialog(BuildContext context, UserPreferencesEntity currentPreferences) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Snooze Duration'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [1, 5, 10, 15, 30]
              .map((duration) => _buildSnoozeOption(context, duration, currentPreferences))
              .toList(),
        ),
      ),
    );
  ...
  // FIXED: Update using current preferences instead of creating new default ones
  Widget _buildSnoozeOption(BuildContext context, int duration, UserPreferencesEntity currentPreferences) {
    return ListTile(
      title: Text('$duration minutes'),
      leading: Radio<int>(
        value: duration,
        groupValue: currentPreferences.reminderSnoozeDuration,
        onChanged: (value) {
          Navigator.pop(context);
          if (value != null) {
            // Update the current preferences with the new duration
            _updatePreference(
              context,
              currentPreferences.copyWith(reminderSnoozeDuration: value), // FIXED: Use current preferences
            );
          }
        },
      ),
